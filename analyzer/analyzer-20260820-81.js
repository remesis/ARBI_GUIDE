(function () {
  ...
  function vitusTailChance(result, actual, classified) {
    const scenarios = result?.scenarios || [];
    const scenarioIndex = scenarios.indexOf(classified);
    const lowerTail = scenarioIndex >= 0 && scenarioIndex <= 1;
    const upperTail = scenarioIndex >= scenarios.length - 2;
    if (!lowerTail && !upperTail) return "";
    const mean = Number(result?.mean);
    const deviation = Number(result?.standardDeviation);
    if (!Number.isFinite(actual) || actual <= 0 || !Number.isFinite(mean) || !Number.isFinite(deviation) || deviation <= 0) return "0.1%";
  ...
  function formatVitusLuckHeadline(result, actual, classified) {
    const chance = vitusTailChance(result, actual, classified);
    return chance ? `${chance} ${classified.label}` : classified.label;
  }
