(function () {
  ...
  function renderComposition(run) {
    ...
    const heading = CORRELATION_LAYOUT_ACTIVE
      ? `<div class="composition-heading"><h3 class="card-title">Enemy composition</h3><span class="composition-despawns">20s Drone despawns: <strong>${fmt(run.dronesDespawned || 0)}</strong></span></div>`
  ...
  function buildCorrelationMetrics(run) {
    ...
    if (!phases.length) return { phases: [], metrics: [], phaseNoun, axisPrefix, fixedIntervals };
    ...
    return { phases, metrics, phaseNoun, axisPrefix, fixedIntervals };
  }
  ...
  function renderCorrelationCard(data) {
    ...
    return `<section class="card correlation-card"><h3 class="card-title">${title}</h3><p class="card-subtitle">${subtitle}</p><div class="correlation-tooltip-stage"><div class="correlation-tooltip" role="status" hidden data-html2canvas-ignore="true"></div></div><div class="correlation-chart-wrap"><svg class="correlation-chart" viewBox="0 0 ${width} ${height}" preserveAspectRatio="xMidYMid meet" aria-label="Per-${h(phaseNoun.toLocaleLowerCase())} metric correlation chart"><g class="correlation-grid">${grid}</g><line class="correlation-average" x1="${pad.left}" y1="${y(0)}" x2="${width - pad.right}" y2="${y(0)}"></line>${series}<line class="correlation-hover-guide" x1="0" y1="${pad.top}" x2="0" y2="${height - pad.bottom}" hidden></line><g class="correlation-axis">${xLabels}</g><g class="correlation-hits">${hits}</g></svg></div><div class="correlation-legend">${legend}</div></section>`;
  }
