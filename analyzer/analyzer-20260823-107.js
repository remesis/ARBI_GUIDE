(function () {
  ...
  function renderClearMap(run, phase) {
    ...
    const cells = phase.items.map((item) => {
      ...
      const visibleDurationMarkup = CORRELATION_LAYOUT_ACTIVE && !phase.defense
        ? visibleDuration.split(/\s+/).map((part) => `<span>${h(part)}</span>`).join("")
        : h(visibleDuration);
      const content = CORRELATION_LAYOUT_ACTIVE
        ? `<span class="clear-cell-content"><strong${phase.defense ? "" : ' class="clear-duration-stack"'}>${visibleDurationMarkup}</strong></span>`
