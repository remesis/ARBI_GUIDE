(function () {
  ...
  function renderClearMap(run, phase) {
    ...
    const cells = phase.items.map((item) => {
      ...
      const droneTooltip = showRotationDrones
        ? ` · ${Number.isFinite(item.drones) ? fmt(item.drones) : "—"} drones`
        : "";
      const tooltip = CORRELATION_LAYOUT_ACTIVE
        ? `${phase.defense ? "Wave" : "Rotation"} ${item.label} · ${shortDuration(item.seconds)}${droneTooltip} · Saturation ${saturation}`
