(function () {
  ...
  function renderVitusInput(view, className = "vitus-input") {
    const classes = className === "vitus-input" ? className : `vitus-input ${className}`;
    return `<input id="actualVitusInput" class="${classes}" type="text" inputmode="numeric" pattern="[0-9]*" maxlength="4" autocomplete="off" aria-label="Actual Vitus" placeholder="####" value="${h(view.actualDigits)}" style="--vitus-actual-color:${view.color}">`;
  }
  ...
  function renderVitusCurve(run, result, view) {
    ...
    const axis = result.scenarios.map((scenario) => {
      ...
      const isMean = Number(scenario.z) === 0;
      const textColor = isMean ? "var(--accent)" : "#8a8b96";
