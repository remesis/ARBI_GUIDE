(function () {
  ...
  function renderClearMap(run, phase) {
    ...
    const showRotationDrones = !phase.defense && ["INTERCEPTION", "SURVIVAL", "MIRROR DEFENSE"].includes(run.missionType);
  ...
  function renderVitusGauge(run, result, view) {
    ...
    const starMarker = [
      clamp(marker[0], 38, width - 38),
      clamp(marker[1], 38, height - 38),
    ];
    const inwardLength = Math.hypot(cx - starMarker[0], cy - starMarker[1]) || 1;
    const secondStar = [
      starMarker[0] + (cx - starMarker[0]) / inwardLength * 22,
      starMarker[1] + (cy - starMarker[1]) / inwardLength * 22,
    ];
    let fill = "";
    ...
    const stops = result.scenarios.map((scenario) => {
      ...
      const isMean = Number(scenario.z) === 0;
    ...
    const tip = view.elite2
      ? `<g filter="url(#vitus-gauge-gold-glow)"><path d="${vitusStarPath(starMarker[0], starMarker[1], 16.5, 7)}" fill="url(#vitus-gauge-gold)" stroke="#0d0e12" stroke-width="1.4" stroke-linejoin="round"/><path d="${vitusStarPath(secondStar[0], secondStar[1], 10, 4.3)}" fill="url(#vitus-gauge-gold-lit)" stroke="#0d0e12" stroke-width="1.3" stroke-linejoin="round"/></g>`
      : view.elite
        ? `<path d="${vitusStarPath(starMarker[0], starMarker[1], 16.5, 7)}" fill="url(#vitus-gauge-gold)" stroke="#0d0e12" stroke-width="1.4" stroke-linejoin="round" filter="url(#vitus-gauge-gold-glow)"/>`
  ...
  function renderVitusCurve(run, result, view) {
    ...
    const wash = washHigh - washLow > .5
      ...
      : "";
    const starX = clamp(actualX, pad + 28, width - pad - 28);
    const starY = clamp(actualY, top + 24, base - 28);
    const starDirection = actualX >= meanX ? -1 : 1;
    const secondStarX = starX + starDirection * 17;
    const secondStarY = starY - 12;
    const tip = view.elite2
      ? `<g filter="url(#vitus-curve-gold-glow)"><path d="${vitusStarPath(starX, starY, 13, 5.5)}" fill="url(#vitus-curve-gold)" stroke="#0d0e12" stroke-width="1.3" stroke-linejoin="round"/><path d="${vitusStarPath(secondStarX, secondStarY, 9, 3.9)}" fill="url(#vitus-curve-gold-lit)" stroke="#0d0e12" stroke-width="1.2" stroke-linejoin="round"/></g>`
      : view.elite
        ? `<path d="${vitusStarPath(starX, starY, 13, 5.5)}" fill="url(#vitus-curve-gold)" stroke="#0d0e12" stroke-width="1.3" stroke-linejoin="round" filter="url(#vitus-curve-gold-glow)"/>`
