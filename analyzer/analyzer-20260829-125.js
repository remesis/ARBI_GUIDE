(function () {
  ...
  const VITUS_AVERAGE_ENDPOINT = "/api/analyzer/spawns/averages";
  ...
  const vitusAverageCache = new Map();
  ...
  function renderReport(run) {
    ...
    if (state.vitusCardStyle !== "original") void ensureTilesetVitusAverage(run);
  }
  ...
  function bindVitusCardInteractions(run, root = document) {
    ...
    card?.querySelectorAll("[data-vitus-style]").forEach((button) => button.addEventListener("click", () => {
      ...
      if (nextStyle !== "original") void ensureTilesetVitusAverage(run);
    }));
  }
  ...
  async function ensureTilesetVitusAverage(run) {
    const nodeKey = String(run?.nodeKey || "");
    if (!/^(?:SolNode|ClanNode|SettlementNode)\d+$/.test(nodeKey)) return;
    let request = vitusAverageCache.get(nodeKey);
    if (!request) {
      request = fetch(`${VITUS_AVERAGE_ENDPOINT}?sol_node=${encodeURIComponent(nodeKey)}`, {
        method: "GET",
        ...
        return payload?.schema === "arbi-analyzer-node-average/v1" && payload?.sol_node === nodeKey && Number.isFinite(rate) && rate > 0
          ? { rate, runs: Number(payload.collected_runs) || 0 }
          : null;
      }).catch(() => null);
      vitusAverageCache.set(nodeKey, request);
    }
    const average = await request;
    if (!average) return;
    run.tilesetAverageVitusRate = average.rate;
    run.tilesetAverageRunCount = average.runs;
    if (state.runs[state.activeIndex] === run && state.vitusCardStyle !== "original") {
