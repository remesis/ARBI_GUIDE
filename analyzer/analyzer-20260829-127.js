(function () {
  ...
  function averageKpi(label, value, delta, formatter, goodWhenHigher = true) {
        ...
        <div class="column left-column">
          <div class="kpi-grid">
            ${CORRELATION_LAYOUT_ACTIVE ? `
              ${averageKpi("Total enemies", fmt(run.enemySpawns), averageDeltas.enemies, (value) => formatSignedVitus(value))}
              ${averageKpi("Drones killed", fmt(run.droneKills), averageDeltas.drones, (value) => formatSignedVitus(value))}
              ${averageKpi("Enemies / min", fmt(enemyRate), averageDeltas.enemiesPerMinute, (value) => formatSignedVitus(value, 1))}
              ${averageKpi("Total duration", duration(run.totalDuration), averageDeltas.duration, formatSignedDuration, false)}
  ...
  function renderVitusRateComparison(view) {
    ...
    return `<div class="vitus-design-rate"><div class="vitus-design-rate-value">${rate} <span>VE/min</span></div><div class="vitus-design-rate-delta ${comparisonClass}">${comparison}</div></div>`;
  }
  ...
  function displayedRunIs(run) {
    if (state.activeSavedId) {
      return state.savedRuns.find((saved) => saved.id === state.activeSavedId)?.run === run;
    }
    return state.runs[state.activeIndex] === run;
  }
  ...
  function renderSaturation(run) {
    ...
    return `<section class="card saturation-card"><h3 class="card-title">Enemy saturation</h3><p class="card-subtitle">Share of run time at each enemy count.</p><div class="metric-bars">${saturation.rows.map((row,index) => { const heat=heatColor(1-index/Math.max(1,saturation.rows.length-1)); return `<div class="metric-row"><span>${h(row.label)}</span><div class="bar-track"><div class="bar-fill" style="--width:${row.percent/max*100}%;--color:${heat.color}"></div></div><strong>${fmt(row.percent,1)}%</strong></div>`; }).join("")}</div><div class="highlight-panel saturation-summary" style="margin-top:13px"><div class="saturation-summary-item"><span class="mini">Time at ${threshold}+ enemies</span><div class="saturation-value-row"><div class="big" style="color:${saturationSummaryColor(saturation.abovePercent)}">${fmt(saturation.abovePercent,1)}%</div>${renderAverageDelta(averageDelta, (value) => `${formatSignedVitus(value, 1)}%`, false)}</div></div><div class="saturation-summary-item telemetry-coverage"><span class="mini">Telemetry coverage</span><div class="big">${telemetryLabel}%</div></div></div></section>`;
  }
