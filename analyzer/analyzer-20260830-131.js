(function () {
  ...
  function effectiveBlessingExpiry(run) {
    ...
    return Number.isFinite(run.resourceBlessingExpiryElapsed) && Number.isFinite(run.resourceBlessingExpiresAt)
  ...
  function buildCorrelationMetrics(run) {
    ...
    const blessingExpiry = effectiveBlessingExpiry(run);
    const canUseClientFreshBlessing = Number.isFinite(run.resourceBlessingExpiryElapsed);
    if (!phases.length) return { phases: [], metrics: [], phaseNoun, axisPrefix, fixedIntervals, blessingExpiry, canUseClientFreshBlessing, clientFreshBlessing: run.clientFreshBlessing === true };
    ...
    return { phases, metrics, phaseNoun, axisPrefix, fixedIntervals, blessingExpiry, canUseClientFreshBlessing, clientFreshBlessing: run.clientFreshBlessing === true };
  }
  ...
  function renderCorrelationCard(data) {
    ...
    const blessingLabel = data.blessingExpiry
      ? `<span class="correlation-blessing-expiry-label">Blessing ran out at: ${h(blessingDuration(data.blessingExpiry.elapsed))}</span>`
      : "";
