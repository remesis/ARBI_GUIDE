(function () {
  ...
  function effectiveBlessedDroneKills(run) {
    if (!run.clientFreshBlessing) return run.blessedDroneKills;
  ...
  function effectiveBlessingExpiry(run) {
    if (run.clientFreshBlessing) {
      if (Number(run.totalDuration) <= RESOURCE_BLESSING_SECONDS) return null;
      return {
        timestamp: Number(run.startTime) + RESOURCE_BLESSING_SECONDS,
    ...
    if (![expiresAt, startTime, endTime].every(Number.isFinite) || expiresAt >= endTime) return null;
  ...
  function reportSpawnMetrics(runs, options = {}) {
    const buildContribution = options.buildContribution || Parser.buildContribution;
