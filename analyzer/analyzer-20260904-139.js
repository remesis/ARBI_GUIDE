(function () {
  ...
  async function importFile(file, settleFocus = false) {
    ...
    try {
      ...
      void reportSpawnMetrics(runs, {
        force: runs.some((run) => !Number.isFinite(run.blessedDroneKills)
  ...
  function buildCorrelationMetrics(run) {
    ...
    if (!phases.length) return { phases: [], metrics: [], phaseNoun, axisPrefix, fixedIntervals, blessingExpiry, canUseClientFreshBlessing: canUseClientFreshBlessingOverride, clientFreshBlessing: run.clientFreshBlessing === true };
    ...
    return { phases, metrics, phaseNoun, axisPrefix, fixedIntervals, blessingExpiry, canUseClientFreshBlessing: canUseClientFreshBlessingOverride, clientFreshBlessing: run.clientFreshBlessing === true };
  }
  ...
  function renderCorrelationCard(data) {
    ...
    const blessingLabel = data.blessingExpiry
      ? `<span class="correlation-blessing-expiry-label">${data.blessingExpiry.expiredBeforeRun ? "Blessing had expired before this run" : `Blessing ran out at: ${h(blessingDuration(data.blessingExpiry.elapsed))}`}</span>`
      : "";
