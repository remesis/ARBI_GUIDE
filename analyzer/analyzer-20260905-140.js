(function () {
  ...
  function renderMinimap(run, points) {
    ...
    const mapWidth = Number.isFinite(Number(config.width)) && Number(config.width) > 0 ? Number(config.width) : 100;
    const mapHeight = Number.isFinite(Number(config.height)) && Number(config.height) > 0 ? Number(config.height) : 100;
    const dimensions = `viewBox="0 0 ${mapWidth} ${mapHeight}"`;
    let bubbles = "";
