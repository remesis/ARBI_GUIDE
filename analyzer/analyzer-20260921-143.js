(function () {
  ...
  const tilesetAverageCache = new Map();
  ...
  async function ensureTilesetAverages(run) {
    ...
    let request = tilesetAverageCache.get(nodeKey);
    if (!request) {
      ...
      tilesetAverageCache.set(nodeKey, request);
    }
    const result = await request;
    if (result?.noBenchmark) tilesetAverageCache.delete(nodeKey);
    const average = fillMissingTilesetAverages(run, result);
    run.tilesetAveragesLoading = false;
    run.tilesetAveragesLoaded = true;
    if (average) {
      run.tilesetAverages = average;
      if (Number.isFinite(average.rate)) run.tilesetAverageVitusRate = average.rate;
