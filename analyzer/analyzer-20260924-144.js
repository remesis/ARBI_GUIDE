(function () {
  ...
  const DISPLAY_PREFERENCES_KEY = "arbi-analyzer-display-v1";
  const TILESET_AVERAGES_VISIBLE_KEY = "arbi-analyzer-tileset-averages-visible-v1";
  const VITUS_CARD_STYLE_KEY = "arbi-analyzer-vitus-card-style-v1";
  const VITUS_CARD_STYLES = Object.freeze(["original", "gauge", "curve"]);
  const SAVED_RUN_DB_NAME = "arbi-analyzer-saved-runs";
  const SAVED_RUN_STORE_NAME = "runs";
  const SAVED_RUN_SCHEMA_VERSION = 1;
  const SAVED_RUN_OMITTED_FIELDS = new Set([
    "allSpawns",
    "agentsFinalized",
    "nonTickingAgents",
    "playerPresence",
    ...
    "sourceName",
    "tilesetAverages",
    "tilesetAveragesLoaded",
    "tilesetAveragesLoading",
    "tilesetAverageVitusRate",
    "tilesetAverageRunCount",
  ]);
  const TILESET_AVERAGE_ENDPOINT = "/api/analyzer/spawns/averages";
  ...
  const state = {
    ...
    savedRuns: [], savedRunsExpanded: false, activeSavedId: "",
    fontStep: initialDisplayPreferences.fontStep,
    spacingStep: initialDisplayPreferences.spacingStep,
    widthStep: initialDisplayPreferences.widthStep,
    hidePlayerNames: loadPlayerNamePrivacy(),
    showTilesetAverages: loadTilesetAverageVisibility(),
    vitusCardStyle: loadVitusCardStyle(),
  ...
  let metricsSubmissionQueue = Promise.resolve();
  let savedRunDatabasePromise = null;
  let savedRunsInitializationPromise = null;
  let savedRunActualVitusWriteQueue = Promise.resolve();
  let savedHeadingPulseTimer = 0;
  const blessingMetricSync = new WeakMap();
  const BLESSING_METRIC_SYNC_DELAY_MS = 2000;
  // Share concurrent reads only. Completed averages must not pin a saved run
  // to an earlier benchmark for the lifetime of this page.
  const tilesetAverageRequests = new Map();
  ...
  function displayStep(value) {
    ...
    return Number.isFinite(parsed) ? Math.max(-3, Math.min(4, Math.round(parsed))) : 0;
  }
  ...
  function loadDisplayPreferences() {
    try {
      ...
      return {
        fontStep: displayStep(saved?.fontStep),
        spacingStep: displayStep(saved?.spacingStep),
        widthStep: displayStep(saved?.widthStep),
      };
    } catch (_) {
      return { fontStep: 0, spacingStep: 0, widthStep: 0 };
    }
  }
  ...
  function saveDisplayPreferences() {
    try {
      localStorage.setItem(DISPLAY_PREFERENCES_KEY, JSON.stringify({
        ...
        spacingStep: state.spacingStep,
        widthStep: state.widthStep,
      }));
    } catch (_) {
      // Display controls still work for this page load when browser storage is unavailable.
    }
  }

  function loadVitusCardStyle() {
    try {
      const saved = localStorage.getItem(VITUS_CARD_STYLE_KEY);
      return VITUS_CARD_STYLES.includes(saved) ? saved : "original";
    } catch (_) {
      return "original";
    }
  }

  function saveVitusCardStyle() {
    try {
      localStorage.setItem(VITUS_CARD_STYLE_KEY, state.vitusCardStyle);
    } catch (_) {
      // The selector still works for this page load when browser storage is unavailable.
    }
  }
  ...
  function normalizedRunDate(run) {
    const value = run?.sourceDate;
    if (!value) return null;
    const date = value instanceof Date ? value : new Date(value);
    return Number.isNaN(date.valueOf()) ? null : date;
  }

  function localRunDate(run) {
    return normalizedRunDate(run)?.toLocaleString(undefined, {
      dateStyle: "medium",
      timeStyle: "short",
    }) || "Date unavailable";
  }

  function sortSavedRuns(records) {
    return records.sort((left, right) => {
      const leftDate = normalizedRunDate(left.run)?.valueOf() ?? Number(left.savedAt || 0);
      const rightDate = normalizedRunDate(right.run)?.valueOf() ?? Number(right.savedAt || 0);
      return leftDate - rightDate || Number(left.savedAt || 0) - Number(right.savedAt || 0);
    });
  }

  function openSavedRunDatabase() {
    if (savedRunDatabasePromise) return savedRunDatabasePromise;
    savedRunDatabasePromise = new Promise((resolve, reject) => {
      if (typeof indexedDB === "undefined") {
        reject(new Error("Saved runs are not available in this browser."));
        return;
      }
      const request = indexedDB.open(SAVED_RUN_DB_NAME, SAVED_RUN_SCHEMA_VERSION);
      request.onupgradeneeded = () => {
        const database = request.result;
        if (!database.objectStoreNames.contains(SAVED_RUN_STORE_NAME)) {
          database.createObjectStore(SAVED_RUN_STORE_NAME, { keyPath: "id" });
        }
      };
      request.onsuccess = () => {
      ...
      request.onerror = () => reject(request.error || new Error("Saved runs could not be opened."));
      request.onblocked = () => reject(new Error("Saved runs are open in another Analyzer tab. Close it and try again."));
    }).catch((error) => {
      savedRunDatabasePromise = null;
      throw error;
    });
    return savedRunDatabasePromise;
  }

  async function readSavedRunRecords() {
    const database = await openSavedRunDatabase();
    return new Promise((resolve, reject) => {
      const transaction = database.transaction(SAVED_RUN_STORE_NAME, "readonly");
      const request = transaction.objectStore(SAVED_RUN_STORE_NAME).getAll();
      request.onsuccess = () => resolve(request.result || []);
      request.onerror = () => reject(request.error || new Error("Saved runs could not be read."));
    });
  }

  async function writeSavedRunRecord(record) {
    const database = await openSavedRunDatabase();
    return new Promise((resolve, reject) => {
      const transaction = database.transaction(SAVED_RUN_STORE_NAME, "readwrite");
      transaction.objectStore(SAVED_RUN_STORE_NAME).put(record);
      transaction.oncomplete = () => resolve();
      transaction.onerror = () => reject(transaction.error || new Error("This run could not be saved."));
      transaction.onabort = () => reject(transaction.error || new Error("This run could not be saved."));
    });
  }

  async function deleteSavedRunRecord(id) {
    const database = await openSavedRunDatabase();
    return new Promise((resolve, reject) => {
      const transaction = database.transaction(SAVED_RUN_STORE_NAME, "readwrite");
      transaction.objectStore(SAVED_RUN_STORE_NAME).delete(id);
      transaction.oncomplete = () => resolve();
      transaction.onerror = () => reject(transaction.error || new Error("This saved run could not be removed."));
      transaction.onabort = () => reject(transaction.error || new Error("This saved run could not be removed."));
    });
  }

  function savedRunSnapshot(run) {
    // The report needs derived timestamps and telemetry summaries to redraw its
    // charts, but never stores the source EE.log text or parser working sets.
    const snapshot = Object.fromEntries(Object.entries(run)
      .filter(([key]) => !SAVED_RUN_OMITTED_FIELDS.has(key)));
    snapshot.sourceDate = normalizedRunDate(run);
    snapshot.actualVitus = cleanVitusDigits(run.actualVitus);
    snapshot.clientFreshBlessing = run.clientFreshBlessing === true;
    return snapshot;
  }

  function reviveSavedRun(record) {
    const run = { ...(record?.run || {}) };
    run.sourceDate = normalizedRunDate(run);
    run.actualVitus = cleanVitusDigits(run.actualVitus);
    run.clientFreshBlessing = run.clientFreshBlessing === true;
    if (Array.isArray(run.liveCounts) && run.liveCounts.length) {
      run.saturation = Parser.helpers.calculateMissionSaturation(run);
    }
    delete run.tilesetAverages;
    delete run.tilesetAveragesLoaded;
    delete run.tilesetAveragesLoading;
    delete run.tilesetAverageVitusRate;
    delete run.tilesetAverageRunCount;
    run.savedRunId = record.id;
    return run;
  }

  function pulseSavedRunsHeading() {
    const heading = $("#savedRunsToggle");
    if (!heading) return;
    if (savedHeadingPulseTimer) clearTimeout(savedHeadingPulseTimer);
    heading.classList.remove("just-saved");
    void heading.offsetWidth;
    heading.classList.add("just-saved");
    savedHeadingPulseTimer = setTimeout(() => heading.classList.remove("just-saved"), 1200);
  }

  function initializeSavedRuns() {
    if (savedRunsInitializationPromise) return savedRunsInitializationPromise;
    savedRunsInitializationPromise = (async () => {
      try {
        const records = await readSavedRunRecords();
        state.savedRuns = sortSavedRuns(records
          .filter((record) => record?.schemaVersion === SAVED_RUN_SCHEMA_VERSION && record?.id && record?.run)
          .map((record) => ({ ...record, run: reviveSavedRun(record) })));
        renderSavedRunList();
      } catch (_) {
        // The Analyzer remains fully usable when persistent browser storage is blocked.
      }
    })();
    return savedRunsInitializationPromise;
  }

  function savedRecordForRun(run) {
    const id = run?.savedRunId || run?.runFingerprint;
    return id ? state.savedRuns.find((record) => record.id === id) || null : null;
  }

  function restoreSavedActualVitus(run) {
    const record = savedRecordForRun(run);
    if (!record) return;
    run.savedRunId = record.id;
    run.actualVitus = cleanVitusDigits(record.run.actualVitus);
  }

  function persistSavedActualVitus(run) {
    const record = savedRecordForRun(run);
    if (!record) return;
    const actualVitus = cleanVitusDigits(run.actualVitus);
    run.savedRunId = record.id;
    run.actualVitus = actualVitus;
    record.run.actualVitus = actualVitus;
    const storedRecord = {
      id: record.id,
      savedAt: record.savedAt,
      schemaVersion: SAVED_RUN_SCHEMA_VERSION,
      run: savedRunSnapshot(record.run),
    };
    savedRunActualVitusWriteQueue = savedRunActualVitusWriteQueue
      .catch(() => {})
      .then(() => writeSavedRunRecord(storedRecord))
      .catch((error) => showToast(error.message || String(error), true));
  }

  async function saveAnalyzedRun(run) {
    const button = $("#saveRunBtn");
    if (button) button.disabled = true;
    try {
      const id = run.runFingerprint || await Parser.fingerprintRun(run);
      const savedAt = Date.now();
      run.runFingerprint = id;
      run.savedRunId = id;
      const record = {
        id,
        savedAt,
        schemaVersion: SAVED_RUN_SCHEMA_VERSION,
        run: savedRunSnapshot(run),
      };
      await writeSavedRunRecord(record);
      state.savedRuns = state.savedRuns.filter((saved) => saved.id !== id);
      state.savedRuns.push({ ...record, run: reviveSavedRun(record) });
      sortSavedRuns(state.savedRuns);
      renderSavedRunList();
      pulseSavedRunsHeading();
      showToast("Run saved locally in this browser.");
    } catch (error) {
  ...
  async function removeSavedRun(id) {
    const record = state.savedRuns.find((saved) => saved.id === id);
    if (!record) return;
    try {
      await deleteSavedRunRecord(id);
      state.savedRuns = state.savedRuns.filter((saved) => saved.id !== id);
      ...
      renderSavedRunList();
      showToast(`${record.run.node || "Run"} removed from saved runs.`);
    } catch (error) {
      showToast(error.message || String(error), true);
    }
  }
  ...
  function squadPlayerEntries(run) {
    const originalNames = [run.host || "Unknown", ...(run.squadmates || [])];
    return squadNames(run).map((name, index) => {
      const companion = run.roboticCompanions?.[originalNames[index]] || "";
      return {
        text: companion ? `${name} (had a ${companion})` : name,
        hasRoboticCompanion: Boolean(companion),
      };
    });
  }
  ...
  function syncSquadPrivacy(root, run) {
    ...
    const entries = squadPlayerEntries(run);
    root.querySelectorAll(".squad-player").forEach((player, index) => {
      const entry = entries[index] || { text: "Unknown", hasRoboticCompanion: false };
      player.textContent = entry.text;
      player.classList.toggle("has-robotic-companion", entry.hasRoboticCompanion);
    });
  }
  ...
  function syncReportHeaderAccent(root = document) {
    ...
    const accent = header?.querySelector(".report-header-accent");
    if (!header || !title || !accent) return;
    ...
    const cssWidth = `${Math.ceil(width)}px`;
    header.style.setProperty("--header-accent-width", cssWidth);
    accent.style.width = cssWidth;
  }

  function measureExportStage(stage, report) {
    ...
    const reportWidth = Math.max(report.offsetWidth || 0, reportRect.width || 0);
    const reportHeight = Math.max(report.offsetHeight || 0, reportRect.height || 0);
    return {
      width: Math.max(1, Math.ceil(paddingLeft + reportWidth + paddingRight)),
      height: Math.max(1, Math.ceil(paddingTop + reportHeight + paddingBottom)),
    };
  }
  ...
  function blessingDuration(seconds) {
    const total = Math.max(0, Math.round(Number(seconds || 0)));
    const hours = Math.floor(total / 3600);
    const minutes = Math.floor((total % 3600) / 60);
    const secs = total % 60;
  ...
  function defenseWaveSeconds(seconds) {
    return Math.max(0, Math.floor(Number(seconds || 0)));
  }

  function defenseWaveScore(seconds) {
    // Wave cells intentionally truncate: 25.999s displays as 25 and remains
    // green. A wave becomes red only when it reaches a full 26 seconds.
    return Number(defenseWaveSeconds(seconds) <= DEFENSE_WAVE_TARGET_SECONDS);
  }
  ...
  function tileElevationBands(config) {
    if (Array.isArray(config?.elevationBands) && config.elevationBands.length === 4
        && config.elevationBands.every(Number.isFinite)) return config.elevationBands;
  ...
  function loadTilesetAverageVisibility() {
    try {
      const saved = localStorage.getItem(TILESET_AVERAGES_VISIBLE_KEY);
  ...
  function saveTilesetAverageVisibility() {
    try {
      localStorage.setItem(TILESET_AVERAGES_VISIBLE_KEY, String(state.showTilesetAverages));
    } catch (_) {
      // The toggle still works for this page load when browser storage is unavailable.
    }
  }

  function averageDeltaClass(delta, goodWhenHigher = true) {
    if (!Number.isFinite(delta)) return "is-pending";
    const favorable = goodWhenHigher ? delta >= 0 : delta <= 0;
    return favorable ? "is-positive" : "is-negative";
  }

  function renderAverageDelta(delta, formatter, goodWhenHigher = true) {
    const label = Number.isFinite(delta) ? `${formatter(delta)} vs AVG` : "⎵ vs AVG";
    return `<div class="metric-average-delta ${averageDeltaClass(delta, goodWhenHigher)}">${h(label)}</div>`;
  }

  function averageKpi(label, value, delta, formatter, goodWhenHigher = true, fallbackNote = "") {
    if (!state.showTilesetAverages) return kpi(label, value, fallbackNote);
    return `<section class="card kpi"><div class="kpi-label">${h(label)}</div><div class="kpi-value">${h(value)}</div>${renderAverageDelta(delta, formatter, goodWhenHigher)}</section>`;
  }

  function comparisonIntervals(run) {
    if (run?.missionType === "DISRUPTION") return Math.max(0, Number(run.activeDuration) || 0) / 360;
    return Math.max(0, Number(run?.rotations) || 0);
  }

  function runAverageDeltas(run, enemyRate) {
    const average = run?.tilesetAverages;
    const intervals = comparisonIntervals(run);
    const expectedTotal = (key) => Number.isFinite(average?.[key]) && intervals > 0
      ? average[key] * intervals
      : null;
    const expectedEnemies = expectedTotal("enemiesPerComparisonInterval");
    const expectedDrones = expectedTotal("dronesPerComparisonInterval");
    const expectedDuration = expectedTotal("durationSecondsPerComparisonInterval");
    return {
      enemies: expectedEnemies === null ? null : Number(run.enemySpawns) - expectedEnemies,
      drones: expectedDrones === null ? null : Number(run.droneKills) - expectedDrones,
      enemiesPerMinute: Number.isFinite(average?.enemiesPerMinute) ? enemyRate - average.enemiesPerMinute : null,
      duration: expectedDuration === null ? null : Number(run.totalDuration) - expectedDuration,
      droneInterval: Number.isFinite(average?.droneIntervalSeconds)
        ? Number(run.avgDroneInterval) - average.droneIntervalSeconds
        : null,
      highEnemyPercent: Number.isFinite(average?.highEnemyPercent)
        && Number(average.highEnemyThreshold) === Number(run?.saturation?.threshold)
        ? Number(run.saturation.abovePercent) - average.highEnemyPercent
        : null,
    };
  }

  function droneClearMetrics(run) {
    const drones = Math.max(0, Number(run?.droneKills) || 0);
    const totalSpawns = Math.max(0, Number(run?.enemySpawns) || 0);
    const nonDroneEnemies = Math.max(0, totalSpawns - drones);
    const enemiesPerDrone = drones > 0 ? nonDroneEnemies / drones : null;
    const observedChancePercent = nonDroneEnemies > 0 ? drones / nonDroneEnemies * 100 : null;
    const efficiencyPercent = Number.isFinite(observedChancePercent) ? observedChancePercent / 20 * 100 : null;
    return { nonDroneEnemies, enemiesPerDrone, observedChancePercent, efficiencyPercent };
  }

  function renderDroneClearEfficiency(run) {
    const metrics = droneClearMetrics(run);
    const efficiencyLabel = Number.isFinite(metrics.efficiencyPercent) ? `${fmt(metrics.efficiencyPercent, 1)}%` : "—";
    const enemiesPerDroneLabel = Number.isFinite(metrics.enemiesPerDrone) ? fmt(metrics.enemiesPerDrone, 2) : "—";
    const observedLabel = Number.isFinite(metrics.observedChancePercent) ? `${fmt(metrics.observedChancePercent, 2)}%` : "—";
    const gaugePercent = clamp(Number(metrics.efficiencyPercent) || 0, 0, 100);
    return `<section class="card kpi drone-clear-efficiency-kpi"><div class="drone-clear-labels"><div class="kpi-label">Clear efficiency</div><div class="kpi-label">Enemies / Drone</div></div><div class="drone-clear-values"><div class="kpi-value">${h(efficiencyLabel)}</div><div class="drone-clear-gauge" role="progressbar" aria-label="Drone clear efficiency" aria-valuemin="0" aria-valuemax="100" aria-valuenow="${gaugePercent}" aria-valuetext="${h(efficiencyLabel)}"><div class="drone-clear-gauge-fill" style="--drone-clear-width:${gaugePercent}%"></div></div><div class="kpi-value">${h(enemiesPerDroneLabel)}</div></div><div class="kpi-note drone-clear-note">${h(observedLabel)} observed · 20% Drone chance before 3/3 cap.</div></section>`;
  }
  ...
  function renderReport(run) {
    ...
    const averageDeltas = runAverageDeltas(run, enemyRate);
    const droneClear = droneClearMetrics(run);
    const perPhase = (phase.items.length ? run.droneKills / phase.items.length : 0);
    const phaseDroneKpi = ["SURVIVAL", "DISRUPTION", "VOID CASCADE"].includes(run.missionType)
    ...
    const date = localRunDate(run);
    const tier = globalThis.ArbitrationTierData?.findTier(run.node);
    const tierBadge = tier
      ? `<span class="tier-badge" style="--tier-rgb:${tier.color.join(",")}"${tier.subname ? ` data-tooltip="${h(tier.subname)}"` : ""}><svg class="tier-badge-frame" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" focusable="false"><rect x="0.5" y="0.5" width="calc(100% - 1px)" height="calc(100% - 1px)" rx="11.5"/></svg><span class="tier-badge-label">${h(tier.name)}</span></span>`
      : "";
    ...
    $("#reportRoot").innerHTML = `
      <header class="report-header">
        ...
        <div>
          ...
          <div class="report-badges">
            ...
            <div class="report-actions">
              <button id="saveRunBtn" class="save-run-button" type="button"><svg width="17" height="17"><use href="#icon-save"></use></svg> SAVE</button>
          ...
          <div class="squad-line">
            ...
            ${squadPlayerEntries(run).map((entry) => `<span class="squad-member"><span class="squad-player${entry.hasRoboticCompanion ? " has-robotic-companion" : ""}">${h(entry.text)}</span><i class="squad-separator" aria-hidden="true"></i></span>`).join("")}
        ...
        <span class="report-header-accent" aria-hidden="true"></span>
      </header>
      <div class="report-grid ${hasSpawnPoints ? "" : "no-spawns"}">
        <div class="column left-column">
          <div class="kpi-grid">
            ${CORRELATION_LAYOUT_ACTIVE ? `
              ${averageKpi("Total enemies", fmt(run.enemySpawns), averageDeltas.enemies, (value) => formatSignedVitus(value), true, "Non-ticking filtered")}
              ${averageKpi("Drones killed", fmt(run.droneKills), averageDeltas.drones, (value) => formatSignedVitus(value), true, "Shield drones")}
              ${averageKpi("Enemies / min", fmt(enemyRate), averageDeltas.enemiesPerMinute, (value) => formatSignedVitus(value, 1), true, "Spawn pace")}
              ${averageKpi("Total duration", duration(run.totalDuration), averageDeltas.duration, formatSignedDuration, false, `${phase.items.length} ${phase.noun}${phase.items.length === 1 ? "" : "s"}`)}
              ${renderDroneClearEfficiency(run)}
            ` : `
              ${kpi("Drones killed", fmt(run.droneKills), "Shield drones")}
              ${kpi("Enemies spawned", fmt(run.enemySpawns), "Non-ticking filtered")}
              ${kpi("Enemies / drone", Number.isFinite(droneClear.enemiesPerDrone) ? fmt(droneClear.enemiesPerDrone, 2) : "—", "Spawned per drone")}
    ...
    syncMinimapStages($("#reportRoot"));
    syncReportHeaderAccent($("#reportRoot"));
    scheduleReportFit();
    $("#saveRunBtn").addEventListener("click", () => saveAnalyzedRun(run));
    ...
    bindVitusCardInteractions(run);
    const needsTilesetAverage = state.showTilesetAverages || state.vitusCardStyle !== "original";
    if (needsTilesetAverage && !run.tilesetAveragesLoaded && !run.tilesetAveragesLoading) void ensureTilesetAverages(run);
  }
  ...
  function renderClearMap(run, phase) {
    ...
    const showRotationDrones = !phase.defense && ["INTERCEPTION", "SURVIVAL", "MIRROR DEFENSE", "INFESTED SALVAGE", "VOID CASCADE"].includes(run.missionType);
    ...
    const cells = phase.items.map((item) => {
      ...
      const visibleDuration = CORRELATION_LAYOUT_ACTIVE && phase.defense
        ? String(defenseWaveSeconds(item.seconds))
        : shortDuration(item.seconds);
      const visibleValue = showRotationDrones
        ? (Number.isFinite(item.drones) ? fmt(item.drones) : "—")
        : visibleDuration;
      const visibleDurationMarkup = CORRELATION_LAYOUT_ACTIVE && !phase.defense && !showRotationDrones
        ? visibleDuration.split(/\s+/).map((part) => `<span>${h(part)}</span>`).join("")
        : h(visibleValue);
      const content = CORRELATION_LAYOUT_ACTIVE
        ? `<span class="clear-cell-content"><strong${!phase.defense && !showRotationDrones ? ' class="clear-duration-stack"' : ""}>${visibleDurationMarkup}</strong></span>`
      ...
      const tooltipDuration = phase.defense ? `${defenseWaveSeconds(item.seconds)}s` : shortDuration(item.seconds);
      const tooltip = CORRELATION_LAYOUT_ACTIVE
        ? `${phase.defense ? "Wave" : "Rotation"} ${item.label} · ${tooltipDuration} · Saturation ${saturation}`
    ...
    const subtitle = phase.defense
      ...
      : (showRotationDrones
        ? "Color is time per rotation, number is Drones per rotation."
  ...
  function dpmChartSeries(run) {
    const hasRotationData = Array.isArray(run.dpmPerRotation) && run.dpmPerRotation.length > 0;
    if (run.missionType === "DISRUPTION" || (run.missionType === "VOID CASCADE" && !hasRotationData)) {
  ...
  function renderVitus(run) {
    ...
    const view = vitusCardView(run, result, actualDigits);
    if (state.vitusCardStyle === "gauge") return renderVitusGauge(run, result, view);
    if (state.vitusCardStyle === "curve") return renderVitusCurve(run, result, view);
    return renderVitusOriginal(run, result, view);
  }

  function renderVitusHeading() {
    const labels = { original: "Original", gauge: "Gauge", curve: "Curve" };
    return `<div class="vitus-card-heading"><h3 class="card-title">Expected Vitus</h3><details class="vitus-style-selector" data-html2canvas-ignore="true"><summary aria-label="Change Expected Vitus card">Change</summary><div class="vitus-style-menu" role="menu" aria-label="Expected Vitus card style">${VITUS_CARD_STYLES.map((style) => `<button class="vitus-style-option${state.vitusCardStyle === style ? " is-selected" : ""}" type="button" role="menuitemradio" aria-checked="${state.vitusCardStyle === style}" data-vitus-style="${style}">${labels[style]}</button>`).join("")}</div></details></div>`;
  }

  function renderVitusInput(view) {
    return `<input id="actualVitusInput" class="vitus-input" type="text" inputmode="numeric" pattern="[0-9]*" maxlength="4" autocomplete="off" aria-label="Actual Vitus" placeholder="####" value="${h(view.actualDigits)}">`;
  }

  function vitusCardView(run, result, actualDigits) {
    const entered = Number(actualDigits);
    const hasActual = Boolean(actualDigits) && Number.isFinite(entered) && entered > 0;
    const actual = hasActual ? entered : result.mean;
    const classified = hasActual ? Parser.classifyVitusScenario(result.scenarios, entered) : result.scenarios[3];
    const color = vitusLuckColor(result.scenarios, classified);
    const rate = actualVitusRate(run);
    const averageRate = Number(run.tilesetAverageVitusRate);
    const hasAverage = Number.isFinite(averageRate) && averageRate > 0;
    const tilesetExpected = hasAverage && Number(run.totalDuration) > 0
      ? averageRate * Number(run.totalDuration) / 60
      : null;
    const shownTail = hasActual ? vitusTailChance(result, entered) : "";
    const tailNumber = Number.parseFloat(shownTail);
    const upperTail = hasActual && entered > result.mean;
    const tailAtMost = (limit) => upperTail && (shownTail.startsWith("<") || (Number.isFinite(tailNumber) && tailNumber <= limit));
    return {
      actualDigits,
      entered,
      hasActual,
      ...
      classified,
      color,
      delta: hasActual ? entered - result.mean : null,
      rate,
      averageRate: hasAverage ? averageRate : null,
      rateDelta: hasActual && rate !== null && hasAverage ? rate - averageRate : null,
      tilesetExpected,
      tailText: hasActual ? formatVitusTailRarity(result, entered) : "",
      elite: tailAtMost(.1),
      elite2: tailAtMost(.01),
    };
  }

  function renderVitusOriginal(run, result, view) {
    return `<section class="card vitus-card vitus-card--original">${renderVitusHeading()}<p class="card-subtitle">${h(vitusAssumptionCopy(run))}</p><div class="highlight-panel vitus-entry-panel"><div class="vitus-entry-group"><span class="vitus-actual"><span class="vitus-entry-label">Actual Vitus</span><span id="vitusDelta" class="mini vitus-delta">${h(formatVitusDelta(result, view.entered))}</span></span>${renderVitusInput(view)}<span id="vitusRate" class="vitus-rate">${h(formatVitusRate(run))}</span></div><div id="vitusLuck" class="vitus-luck" style="--luck-color:${view.color}"><strong>${h(view.classified.label)}</strong><div class="mini vitus-tail">${h(view.tailText)}</div></div></div><table class="vitus-table"><thead><tr><th>CHANCE</th><th>TOTAL</th><th>LUCK LEVEL</th></tr></thead><tbody>${result.scenarios.map((scenario) => `<tr class="${scenario === view.classified ? "active" : ""}"><td>${h(formatVitusScenarioChance(scenario.chance))}</td><td><strong>${fmt(scenario.total)}</strong></td><td>${h(scenario.label)}</td></tr>`).join("")}</tbody></table></section>`;
  }

  function actualVitusRate(run) {
    const raw = String(run.actualVitus ?? "").trim();
    const actual = Number(raw);
    const seconds = Number(run.totalDuration);
    return raw && Number.isFinite(actual) && actual >= 0 && Number.isFinite(seconds) && seconds > 0
      ? actual * 60 / seconds
      : null;
  }

  function formatSignedVitus(value, digits = 0) {
    if (!Number.isFinite(value)) return "—";
    const factor = 10 ** digits;
    const rounded = Math.round(value * factor) / factor;
    const normalized = Object.is(rounded, -0) ? 0 : rounded;
    return `${normalized >= 0 ? "+" : ""}${fmt(normalized, digits)}`;
  }

  function formatSignedDuration(value) {
    if (!Number.isFinite(value)) return "—";
    const rounded = Math.round(value);
    return `${rounded >= 0 ? "+" : "-"}${shortDuration(Math.abs(rounded))}`;
  }

  function vitusPolar(cx, cy, radius, degrees) {
    const radians = (degrees - 90) * Math.PI / 180;
    return [cx + radius * Math.cos(radians), cy + radius * Math.sin(radians)];
  }

  function vitusArcPath(cx, cy, radius, start, end) {
    const first = vitusPolar(cx, cy, radius, end);
    const last = vitusPolar(cx, cy, radius, start);
    const large = Math.abs(end - start) <= 180 ? 0 : 1;
    return `M ${first[0].toFixed(2)} ${first[1].toFixed(2)} A ${radius} ${radius} 0 ${large} 0 ${last[0].toFixed(2)} ${last[1].toFixed(2)}`;
  }

  function vitusStarPath(cx, cy, outer, inner) {
    let path = "";
    for (let index = 0; index < 10; index += 1) {
      const radius = index % 2 ? inner : outer;
      const angle = index * Math.PI / 5 - Math.PI / 2;
      path += `${index ? "L" : "M"}${(cx + radius * Math.cos(angle)).toFixed(2)} ${(cy + radius * Math.sin(angle)).toFixed(2)} `;
    }
    return `${path}Z`;
  }

  function vitusGaugeFraction(result, value) {
    const deviation = Math.max(.0001, Number(result.standardDeviation) || 0);
    const span = 3.090232306167813;
    return clamp((value - (result.mean - span * deviation)) / (2 * span * deviation), 0, 1);
  }

  function vitusScenarioCutoff(scenario) {
    return Number.isFinite(Number(scenario?.cutoff)) ? Number(scenario.cutoff) : Number(scenario?.total) || 0;
  }

  function vitusGaugeTierColor(result, fraction) {
    const scenarios = result.scenarios;
    const bounds = [0, 1, 2, 4, 5, 6].map((index) => vitusScenarioCutoff(scenarios[index]));
    const nodes = [[0, 0], [vitusGaugeFraction(result, bounds[0]), 0]];
    for (let tier = 1; tier <= 5; tier += 1) {
      nodes.push([vitusGaugeFraction(result, (bounds[tier - 1] + bounds[tier]) / 2), tier / (scenarios.length - 1)]);
    }
    nodes.push([vitusGaugeFraction(result, bounds[5]), 1], [1, 1]);
    for (let index = 0; index < nodes.length - 1; index += 1) {
      const first = nodes[index];
      const next = nodes[index + 1];
      if (fraction <= next[0] || index === nodes.length - 2) {
        const progress = next[0] > first[0] ? clamp((fraction - first[0]) / (next[0] - first[0]), 0, 1) : 0;
        return heatColor(first[1] + (next[1] - first[1]) * progress).color;
      }
    }
    return heatColor(1).color;
  }

  function renderVitusRateComparison(view) {
    const rate = view.rate === null ? "⎵" : fmt(view.rate, 1);
    const comparison = view.rateDelta === null ? "⎵ vs AVG" : `${formatSignedVitus(view.rateDelta, 1)} vs AVG`;
    const comparisonClass = view.rateDelta === null ? "is-pending" : view.rateDelta >= 0 ? "is-positive" : "is-negative";
    const delta = state.showTilesetAverages
      ? `<div class="vitus-design-rate-delta ${comparisonClass}">${comparison}</div>`
      : "";
    return `<div class="vitus-design-rate"><div class="vitus-design-rate-value">${rate} <span>VE/min</span></div>${delta}</div>`;
  }

  function renderVitusGauge(run, result, view) {
    const width = 490;
    const height = 234;
    const cx = width / 2;
    const cy = 154;
    ...
    const start = -118;
    const end = 118;
    const angleAt = (fraction) => start + fraction * (end - start);
    const actualFraction = vitusGaugeFraction(result, view.actual);
    const actualAngle = angleAt(actualFraction);
    const marker = vitusPolar(cx, cy, radius, actualAngle);
    const tangentAngle = (actualAngle - 90) * Math.PI / 180;
    const tangent = [-Math.sin(tangentAngle), Math.cos(tangentAngle)];
    let fill = "";
    for (let index = 0; index < 90; index += 1) {
      const fractionStart = index / 90;
      if (fractionStart >= actualFraction) break;
      const fractionEnd = Math.min((index + 1) / 90, actualFraction);
      fill += `<path d="${vitusArcPath(cx, cy, radius, angleAt(fractionStart), angleAt(fractionEnd) + .35)}" fill="none" stroke="${vitusGaugeTierColor(result, (fractionStart + fractionEnd) / 2)}" stroke-width="17" stroke-linecap="round"/>`;
    }
    const stops = result.scenarios.map((scenario) => {
      const angle = angleAt(vitusGaugeFraction(result, scenario.total));
      const inner = vitusPolar(cx, cy, radius - 13, angle);
      const outer = vitusPolar(cx, cy, radius + 10, angle);
      const isMean = scenario.label === "Average";
      const direction = Math.cos((angle - 90) * Math.PI / 180);
      const side = Math.abs(direction) >= .25;
      const label = vitusPolar(cx, cy, radius + (side ? 19 : 26), angle);
      const anchor = side ? (direction > 0 ? "start" : "end") : "middle";
      return `<line x1="${inner[0].toFixed(1)}" y1="${inner[1].toFixed(1)}" x2="${outer[0].toFixed(1)}" y2="${outer[1].toFixed(1)}" stroke="${isMean ? "#f5f5f7" : "#4a4b56"}" stroke-width="${isMean ? 2.5 : 1.6}" stroke-linecap="round"/><text x="${label[0].toFixed(1)}" y="${label[1].toFixed(1)}" text-anchor="${anchor}" dominant-baseline="middle" fill="${isMean ? "#f5f5f7" : "#8a8b96"}" font-size="${isMean ? 16 : 14}" font-weight="${isMean ? 900 : 800}" font-family="Inter,system-ui,sans-serif">${h(scenario.total)}</text>`;
    }).join("");
    const tip = view.elite2
      ? `<g filter="url(#vitus-gauge-gold-glow)"><path d="${vitusStarPath(marker[0], marker[1], 16.5, 7)}" fill="url(#vitus-gauge-gold)" stroke="#0d0e12" stroke-width="1.4" stroke-linejoin="round"/><path d="${vitusStarPath(marker[0] + tangent[0] * 19, marker[1] + tangent[1] * 19, 10, 4.3)}" fill="url(#vitus-gauge-gold-lit)" stroke="#0d0e12" stroke-width="1.3" stroke-linejoin="round"/></g>`
      : view.elite
        ? `<path d="${vitusStarPath(marker[0], marker[1], 16.5, 7)}" fill="url(#vitus-gauge-gold)" stroke="#0d0e12" stroke-width="1.4" stroke-linejoin="round" filter="url(#vitus-gauge-gold-glow)"/>`
        : `<circle cx="${marker[0].toFixed(1)}" cy="${marker[1].toFixed(1)}" r="11.5" fill="${view.color}" stroke="#0d0e12" stroke-width="1" filter="url(#vitus-gauge-glow)"/><circle cx="${marker[0].toFixed(1)}" cy="${marker[1].toFixed(1)}" r="4.4" fill="#0d0e12"/>`;
    const tilesetExpected = view.tilesetExpected === null ? "⎵" : fmt(view.tilesetExpected);
    const expectedDelta = view.delta === null ? "" : ` <span style="color:${view.color}">${formatSignedVitus(view.delta)}</span>`;
    return `<section class="card vitus-card vitus-card--visual vitus-card--gauge">${renderVitusHeading()}<p class="card-subtitle">${h(vitusAssumptionCopy(run))}</p><div class="vitus-design-entry-panel"><div class="vitus-design-entry-row"><span class="vitus-design-entry-label">Actual Vitus</span>${renderVitusInput(view)}</div>${renderVitusRateComparison(view)}</div><div class="vitus-visual-fill"><svg viewBox="0 0 ${width} ${height}" preserveAspectRatio="xMidYMax meet" aria-label="Expected Vitus luck gauge"><defs><filter id="vitus-gauge-glow" x="-60%" y="-60%" width="220%" height="220%"><feGaussianBlur stdDeviation="7" result="b"/><feMerge><feMergeNode in="b"/><feMergeNode in="SourceGraphic"/></feMerge></filter><linearGradient id="vitus-gauge-gold" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="#ffe98a"/><stop offset=".5" stop-color="#ffc93c"/><stop offset="1" stop-color="#e39a12"/></linearGradient><linearGradient id="vitus-gauge-gold-lit" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="#fffbe6"/><stop offset=".55" stop-color="#ffe27a"/><stop offset="1" stop-color="#ffc02e"/></linearGradient><filter id="vitus-gauge-gold-glow" x="-140%" y="-140%" width="380%" height="380%"><feDropShadow dx="0" dy="0" stdDeviation="6" flood-color="#ffc93c" flood-opacity=".95"/></filter></defs><path d="${vitusArcPath(cx, cy, radius, start, end)}" fill="none" stroke="#23242b" stroke-width="17" stroke-linecap="round"/><g filter="url(#vitus-gauge-glow)">${fill}</g>${stops}${tip}<text x="${cx}" y="${cy - 4}" text-anchor="middle" fill="${view.color}" font-size="28" font-weight="900" font-family="Inter,system-ui,sans-serif">${h(view.classified.label)}</text><text x="${cx}" y="${cy + 22}" text-anchor="middle" fill="#9a9ba6" font-size="14.5" font-weight="700" font-family="Inter,system-ui,sans-serif">${h(view.tailText || "Awaiting Actual Vitus")}</text></svg></div><div class="vitus-gauge-legend"><span>Tileset AVG expected <b>${tilesetExpected}</b></span><span>Expected <b>${fmt(result.mean)}</b>${expectedDelta}</span></div></section>`;
  }

  let vitusMeasureContext = null;

  function measureVitusText(value, font) {
    if (!vitusMeasureContext) vitusMeasureContext = document.createElement("canvas").getContext("2d");
    vitusMeasureContext.font = font;
    return vitusMeasureContext.measureText(String(value)).width;
  }

  function renderVitusCurve(run, result, view) {
    const width = 490;
    const curveHeight = 176;
    const axisHeight = 44;
    const height = curveHeight + axisHeight;
    const pad = 22;
    const base = curveHeight - 4;
    const top = 16;
    const deviation = Math.max(.0001, Number(result.standardDeviation) || 0);
    const low = result.mean - 3.1 * deviation;
    const high = result.mean + 3.1 * deviation;
    const xAt = (value) => pad + (clamp(value, low, high) - low) / (high - low) * (width - pad * 2);
    const density = (value) => Math.exp(-.5 * ((value - result.mean) / deviation) ** 2);
    const yAt = (value) => base - density(clamp(value, low, high)) * (base - top);
    const points = [];
    for (let index = 0; index <= 160; index += 1) {
      const value = low + (high - low) * index / 160;
      points.push([xAt(value), yAt(value)]);
    }
    const line = points.map((point, index) => `${index ? "L" : "M"} ${point[0].toFixed(1)} ${point[1].toFixed(1)}`).join(" ");
    const area = `${line} L ${xAt(high).toFixed(1)} ${base} L ${xAt(low).toFixed(1)} ${base} Z`;
    const actualX = xAt(view.actual);
    const actualY = yAt(view.actual);
    const meanX = xAt(result.mean);
    const washLow = Math.min(meanX, actualX);
    const washHigh = Math.max(meanX, actualX);
    const edgeActual = [actualX, actualY];
    const edgeMean = [meanX, yAt(result.mean)];
    const washPoints = [actualX <= meanX ? edgeActual : edgeMean]
      .concat(points.filter((point) => point[0] > washLow && point[0] < washHigh))
      .concat([actualX <= meanX ? edgeMean : edgeActual]);
    const wash = washHigh - washLow > .5
      ? `${washPoints.map((point, index) => `${index ? "L" : "M"} ${point[0].toFixed(2)} ${point[1].toFixed(2)}`).join(" ")} L ${washHigh.toFixed(2)} ${base} L ${washLow.toFixed(2)} ${base} Z`
      : "";
    const tip = view.elite2
      ? `<g filter="url(#vitus-curve-gold-glow)"><path d="${vitusStarPath(actualX, actualY, 13, 5.5)}" fill="url(#vitus-curve-gold)" stroke="#0d0e12" stroke-width="1.3" stroke-linejoin="round"/><path d="${vitusStarPath(actualX + 11, actualY + 11, 9, 3.9)}" fill="url(#vitus-curve-gold-lit)" stroke="#0d0e12" stroke-width="1.2" stroke-linejoin="round"/></g>`
      : view.elite
        ? `<path d="${vitusStarPath(actualX, actualY, 13, 5.5)}" fill="url(#vitus-curve-gold)" stroke="#0d0e12" stroke-width="1.3" stroke-linejoin="round" filter="url(#vitus-curve-gold-glow)"/>`
        : `<circle cx="${actualX.toFixed(1)}" cy="${actualY.toFixed(1)}" r="9" fill="${view.color}" stroke="#0d0e12" stroke-width="1" filter="url(#vitus-curve-glow)"/><circle cx="${actualX.toFixed(1)}" cy="${actualY.toFixed(1)}" r="3.6" fill="#0d0e12"/>`;
    const stem = 12;
    const axis = result.scenarios.map((scenario) => {
      const x = xAt(scenario.total);
      const isMean = scenario.label === "Average";
      const textColor = isMean ? "#72c7ff" : "#8a8b96";
      const ruleColor = isMean ? "#6f707c" : "#43444e";
      const textX = (x - 4).toFixed(1);
      const textY = stem - 3;
      const textLength = measureVitusText(scenario.total, `${isMean ? 900 : 800} 14px Inter, system-ui, sans-serif`);
      const leader = (textLength + .7) / Math.SQRT2;
      return `<line x1="${x.toFixed(1)}" y1="0" x2="${x.toFixed(1)}" y2="${stem}" stroke="${ruleColor}" stroke-width="1.4"/><line x1="${x.toFixed(1)}" y1="${stem}" x2="${(x - leader).toFixed(1)}" y2="${(stem + leader).toFixed(1)}" stroke="${ruleColor}" stroke-width="1.4"/><text x="${textX}" y="${textY}" text-anchor="end" transform="rotate(-45 ${textX} ${textY})" fill="${textColor}" font-size="14" font-weight="${isMean ? 900 : 800}" font-family="Inter,system-ui,sans-serif">${h(scenario.total)}</text>`;
    }).join("");
    const tilesetExpected = view.tilesetExpected === null ? "⎵" : fmt(view.tilesetExpected);
    return `<section class="card vitus-card vitus-card--visual vitus-card--curve">${renderVitusHeading()}<p class="card-subtitle">${h(vitusAssumptionCopy(run))}</p><div class="vitus-design-entry-panel"><div class="vitus-design-entry-row"><span class="vitus-design-entry-stack"><span class="vitus-design-entry-label">Actual Vitus</span><span class="vitus-design-entry-delta">${h(formatVitusDelta(result, view.entered))}</span></span>${renderVitusInput(view)}</div><div class="vitus-curve-verdict"><strong style="color:${view.color}">${h(view.classified.label)}</strong><span>${h(view.tailText)}</span></div></div><div class="vitus-curve-summary"><div>Tileset AVG expected <b>${tilesetExpected}</b></div>${renderVitusRateComparison(view)}</div><div class="vitus-visual-fill"><svg viewBox="0 0 ${width} ${height}" preserveAspectRatio="none" aria-label="Expected Vitus distribution curve"><defs><linearGradient id="vitus-curve-area" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="#5a5b68" stop-opacity=".55"/><stop offset="1" stop-color="#5a5b68" stop-opacity="0"/></linearGradient><linearGradient id="vitus-curve-wash" gradientUnits="userSpaceOnUse" x1="${meanX.toFixed(1)}" y1="0" x2="${actualX.toFixed(1)}" y2="0"><stop offset="0" stop-color="${view.color}" stop-opacity=".06"/><stop offset="1" stop-color="${view.color}" stop-opacity=".85"/></linearGradient><linearGradient id="vitus-curve-gold" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="#ffe98a"/><stop offset=".5" stop-color="#ffc93c"/><stop offset="1" stop-color="#e39a12"/></linearGradient><linearGradient id="vitus-curve-gold-lit" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="#fffbe6"/><stop offset=".55" stop-color="#ffe27a"/><stop offset="1" stop-color="#ffc02e"/></linearGradient><filter id="vitus-curve-gold-glow" x="-140%" y="-140%" width="380%" height="380%"><feDropShadow dx="0" dy="0" stdDeviation="5" flood-color="#ffc93c" flood-opacity=".95"/></filter><filter id="vitus-curve-glow" x="-140%" y="-140%" width="380%" height="380%"><feDropShadow dx="0" dy="0" stdDeviation="4" flood-color="${view.color}" flood-opacity=".85"/></filter></defs><path d="${area}" fill="url(#vitus-curve-area)"/>${wash ? `<path d="${wash}" fill="url(#vitus-curve-wash)"/>` : ""}<path d="${line}" fill="none" stroke="#8f909c" stroke-width="1.8"/><line x1="${meanX.toFixed(1)}" y1="${yAt(result.mean).toFixed(1)}" x2="${meanX.toFixed(1)}" y2="${base}" stroke="#f5f5f7" stroke-width="1.4" stroke-dasharray="4 4" opacity=".7"/><line x1="${actualX.toFixed(1)}" y1="${actualY.toFixed(1)}" x2="${actualX.toFixed(1)}" y2="${base}" stroke="${view.color}" stroke-width="2.6"/><line x1="${pad}" y1="${base}" x2="${width - pad}" y2="${base}" stroke="#33343d" stroke-width="1.4"/>${tip}<g transform="translate(0 ${curveHeight})">${axis}</g></svg></div></section>`;
  }

  function bindVitusCardInteractions(run, root = document) {
    const card = root.querySelector(".vitus-card");
    const input = card?.querySelector("#actualVitusInput");
    if (input) input.addEventListener("input", () => {
      const digits = cleanVitusDigits(input.value);
      run.actualVitus = digits;
      persistSavedActualVitus(run);
      replaceVitusCard(run, { focusInput: true });
    });
    card?.querySelectorAll("[data-vitus-style]").forEach((button) => button.addEventListener("click", () => {
      const nextStyle = button.dataset.vitusStyle;
      if (!VITUS_CARD_STYLES.includes(nextStyle)) return;
      if (nextStyle === state.vitusCardStyle) {
        button.closest("details")?.removeAttribute("open");
        return;
      }
      state.vitusCardStyle = nextStyle;
      saveVitusCardStyle();
      replaceVitusCard(run);
      if (!run.tilesetAveragesLoaded && !run.tilesetAveragesLoading) void ensureTilesetAverages(run);
    }));
  }

  function replaceVitusCard(run, { focusInput = false } = {}) {
    const current = $(".vitus-card", $("#reportRoot"));
    if (!current) return;
    const template = document.createElement("template");
    template.innerHTML = renderVitus(run).trim();
    const next = template.content.firstElementChild;
    current.replaceWith(next);
    bindVitusCardInteractions(run, next.parentElement);
    scheduleReportFit();
    if (focusInput) focusActualVitusEntry();
  }

  function currentDisplayedRun() {
    if (state.activeSavedId) {
      return state.savedRuns.find((saved) => saved.id === state.activeSavedId)?.run || null;
    }
    return state.runs[state.activeIndex] || null;
  }

  function displayedRunIs(run) {
    return currentDisplayedRun() === run;
  }

  function syncTilesetAverageToggle() {
    const button = $("#tilesetAverageToggle");
    if (!button) return;
    button.classList.toggle("is-active", state.showTilesetAverages);
    button.setAttribute("aria-pressed", String(state.showTilesetAverages));
  }
  ...
  function singleRunTilesetAverages(run) {
    const intervals = comparisonIntervals(run);
    const activeSeconds = Number(run?.activeDuration);
    return {
      rate: null,
      runs: 0,
      isSingleRunFallback: true,
      dronesPerComparisonInterval: intervals > 0 ? Number(run.droneKills) / intervals : null,
      enemiesPerComparisonInterval: intervals > 0 ? Number(run.enemySpawns) / intervals : null,
      enemiesPerMinute: activeSeconds > 0 ? Number(run.enemySpawns) * 60 / activeSeconds : null,
      durationSecondsPerComparisonInterval: intervals > 0 ? Number(run.totalDuration) / intervals : null,
      droneIntervalSeconds: finiteAverage(run?.avgDroneInterval),
      highEnemyPercent: finiteAverage(run?.saturation?.abovePercent),
      highEnemyThreshold: finiteAverage(run?.saturation?.threshold),
    };
  }

  function fillMissingTilesetAverages(run, average) {
    if (!average) return null;
    const fallback = singleRunTilesetAverages(run);
    const resolved = { rate: null, runs: 0, ...average };
    const metricKeys = [
      "dronesPerComparisonInterval",
      "enemiesPerComparisonInterval",
      "enemiesPerMinute",
      "durationSecondsPerComparisonInterval",
      "droneIntervalSeconds",
      "highEnemyPercent",
      "highEnemyThreshold",
    ];
    let usedFallback = average.noBenchmark === true;
    for (const key of metricKeys) {
      if (Number.isFinite(resolved[key])) continue;
      resolved[key] = fallback[key];
      usedFallback = true;
    }
    resolved.isSingleRunFallback = usedFallback;
    return resolved;
  }

  async function ensureTilesetAverages(run) {
    if (!run || run.tilesetAveragesLoading) return;
    const nodeKey = String(run?.nodeKey || "");
    if (!/^(?:SolNode|ClanNode|SettlementNode)\d+$/.test(nodeKey)) {
      run.tilesetAveragesLoaded = true;
      return;
    }
    run.tilesetAveragesLoading = true;
    let request = tilesetAverageRequests.get(nodeKey);
    if (!request) {
      request = fetch(`${TILESET_AVERAGE_ENDPOINT}?sol_node=${encodeURIComponent(nodeKey)}`, {
        method: "GET",
        headers: { Accept: "application/json" },
        credentials: "same-origin",
        cache: "no-store",
      }).then(async (response) => {
        if (response.status === 404) return { noBenchmark: true };
        if (!response.ok) return null;
        const payload = await response.json();
        const rate = Number(payload?.expected_ve_per_minute);
        return payload?.schema === "arbi-analyzer-node-average/v1" && payload?.sol_node === nodeKey && Number.isFinite(rate) && rate > 0
          ? {
            rate,
            runs: Number(payload.collected_runs) || 0,
            dronesPerComparisonInterval: finiteAverage(payload.drones_per_comparison_interval),
            enemiesPerComparisonInterval: finiteAverage(payload.enemies_per_comparison_interval),
            enemiesPerMinute: finiteAverage(payload.enemies_per_minute),
            durationSecondsPerComparisonInterval: finiteAverage(payload.duration_seconds_per_comparison_interval),
            droneIntervalSeconds: finiteAverage(payload.drone_interval_seconds),
            highEnemyPercent: finiteAverage(payload.high_enemy_percent),
            highEnemyThreshold: finiteAverage(payload.high_enemy_threshold),
          }
          : null;
      }).catch(() => null);
      tilesetAverageRequests.set(nodeKey, request);
    }
    const result = await request;
    if (tilesetAverageRequests.get(nodeKey) === request) tilesetAverageRequests.delete(nodeKey);
    const average = fillMissingTilesetAverages(run, result);
    run.tilesetAveragesLoading = false;
    run.tilesetAveragesLoaded = true;
    if (average) {
      run.tilesetAverages = average;
      run.tilesetAverageVitusRate = Number.isFinite(average.rate) ? average.rate : null;
      run.tilesetAverageRunCount = average.runs;
    }
    if (!average && state.activeSavedId && displayedRunIs(run)) {
      showToast(run.tilesetAverages
        ? "Could not refresh tileset averages. Showing the last available averages; reopen the run to retry."
        : "Tileset averages are unavailable right now. Reopen the run to retry.", true);
    }
    if (average && displayedRunIs(run)) {
      const focusInput = document.activeElement?.id === "actualVitusInput";
      renderReport(run);
      if (focusInput) focusActualVitusEntry();
    }
  }
  ...
  function computeRunVitus(run) {
    return Parser.computeVitus(run.droneKills, run.rotations, run.missionType, effectiveBlessedDroneKills(run));
  }

  function effectiveBlessedDroneKills(run) {
    if (!run.clientFreshBlessing && Number.isFinite(run.blessedDroneKills)) return run.blessedDroneKills;
    if (Number(run.totalDuration) <= RESOURCE_BLESSING_SECONDS) return run.droneKills;
    const expiresAt = Number(run.startTime) + RESOURCE_BLESSING_SECONDS;
    const timestamps = Array.isArray(run.droneTimestamps) ? run.droneTimestamps : [];
    if (Number.isFinite(expiresAt) && timestamps.length) {
      return Math.min(run.droneKills, timestamps.filter((timestamp) => timestamp <= expiresAt).length);
    }
    return Math.round(run.droneKills * RESOURCE_BLESSING_SECONDS / Math.max(RESOURCE_BLESSING_SECONDS, Number(run.totalDuration) || 0));
  }

  function effectiveBlessingExpiry(run) {
    if (!run.clientFreshBlessing) {
      const expiresAt = Number(run.resourceBlessingExpiresAt);
      const startTime = Number(run.startTime);
      const endTime = Number(run.endTime);
      if ([expiresAt, startTime, endTime].every(Number.isFinite)) {
        if (expiresAt >= endTime) return null;
        const expiredBeforeRun = expiresAt <= startTime;
        return {
          timestamp: expiredBeforeRun ? startTime : expiresAt,
          elapsed: expiredBeforeRun ? 0 : expiresAt - startTime,
          expiredBeforeRun,
        };
      }
      if (Number.isFinite(run.blessedDroneKills)) return null;
    }
    const totalDuration = Number(run.totalDuration);
    const startTime = Number(run.startTime);
    if (!Number.isFinite(totalDuration) || !Number.isFinite(startTime) || totalDuration <= RESOURCE_BLESSING_SECONDS) {
      return null;
    }
    return {
      timestamp: startTime + RESOURCE_BLESSING_SECONDS,
      elapsed: RESOURCE_BLESSING_SECONDS,
      assumedFromMissionStart: !run.clientFreshBlessing,
    };
  }

  function canUseClientFreshBlessing(run) {
    if (run.resourceBlessingRefreshUnconfirmed) return true;
    const expiresAt = Number(run.resourceBlessingExpiresAt);
    const endTime = Number(run.endTime);
    return Number.isFinite(expiresAt) && Number.isFinite(endTime) && expiresAt < endTime;
  }

  function vitusAssumptionCopy(run) {
    if (run.resourceBlessingRefreshUnconfirmed && !run.clientFreshBlessing) {
      return Number.isFinite(run.resourceBlessingConfirmedAt)
        ? "New Drop Blessing not confirmed before launch; using the previous confirmed timer."
        : "Refresh not confirmed before launch; Drop Blessing assumed fresh at mission start for up to 3h.";
    }
    const blessingExpiry = effectiveBlessingExpiry(run);
    if (run.clientFreshBlessing && !blessingExpiry) {
      return "Blessing, Both Boosters and Resourceful Retriever.";
    }
    if (blessingExpiry?.expiredBeforeRun) {
      return "Both Boosters and Resourceful Retriever.";
    }
    if (blessingExpiry?.assumedFromMissionStart) {
      return `Both Boosters, Resourceful Retriever, and Drop Blessing assumed fresh at mission start until ${blessingDuration(blessingExpiry.elapsed)}.`;
    }
    if (blessingExpiry) {
      return `Both Boosters, Resourceful Retriever, and Drop Blessing until ${blessingDuration(blessingExpiry.elapsed)}.`;
    }
  ...
  function updateVitusActual(run) {
    replaceVitusCard(run, { focusInput: document.activeElement?.id === "actualVitusInput" });
  }
  ...
  function renderSaturation(run) {
    ...
    const averageDelta = runAverageDeltas(run, 0).highEnemyPercent;
    ...
    const comparison = state.showTilesetAverages
      ? renderAverageDelta(averageDelta, (value) => `${formatSignedVitus(value, 1)}%`, false)
      : "";
    return `<section class="card saturation-card"><h3 class="card-title">Enemy saturation</h3><p class="card-subtitle">Share of run time at each enemy count.</p><div class="metric-bars">${saturation.rows.map((row,index) => { const heat=heatColor(1-index/Math.max(1,saturation.rows.length-1)); return `<div class="metric-row"><span>${h(row.label)}</span><div class="bar-track"><div class="bar-fill" style="--width:${row.percent/max*100}%;--color:${heat.color}"></div></div><strong>${fmt(row.percent,1)}%</strong></div>`; }).join("")}</div><div class="highlight-panel saturation-summary" style="margin-top:13px"><div class="saturation-summary-item"><span class="mini">Time at ${threshold}+ enemies</span><div class="saturation-value-row"><div class="big" style="color:${saturationSummaryColor(saturation.abovePercent)}">${fmt(saturation.abovePercent,1)}%</div>${comparison}</div></div><div class="saturation-summary-item telemetry-coverage"><span class="mini">Telemetry coverage</span><div class="big">${telemetryLabel}%</div></div></div></section>`;
  }
  ...
  function renderCadence(run) {
    ...
    const intervalDelta = runAverageDeltas(run, 0).droneInterval;
    const intervalComparison = state.showTilesetAverages
      ? renderAverageDelta(intervalDelta, (value) => `${formatSignedVitus(value, 2)}s`, false)
      : "";
    return `<section class="card"><h3 class="card-title">Drone cadence</h3><p class="card-subtitle">Share of wait time spent waiting this long for the next drone.</p><div class="metric-bars">${cadence.rows.map((row,index) => { const heat=heatColor(1-index/Math.max(1,cadence.rows.length-1)); return `<div class="metric-row"><span>${h(row.label === "8-12s" ? "8–<12s" : row.label)}</span><div class="bar-track"><div class="bar-fill" style="--width:${row.percent/max*100}%;--color:${heat.color}"></div></div><strong>${fmt(row.percent,1)}%</strong></div>`; }).join("")}</div><div class="highlight-panel cadence-summary" style="margin-top:13px"><div class="cadence-summary-item cadence-summary-interval"><span class="mini">Drone Interval</span><div class="cadence-interval-value-row"><div class="big">${fmt(run.avgDroneInterval, 2)}s</div>${intervalComparison}</div></div><div class="cadence-summary-item cadence-summary-dry"><span class="mini">Dry ≥12s</span><div class="big">${fmt(cadence.droughtPercent,1)}%</div></div><div class="cadence-summary-item cadence-summary-peak"><span class="mini">Peak / 10s</span><div class="big">${fmt(peak.count)}</div></div></div></section>`;
  }
  ...
  function isExcludedCompositionAgent(value) {
    const normalized = String(value).replace(/[^a-z0-9]/gi, "").replace(/agent$/i, "").toLowerCase();
    return normalized === "summonmotorcycle"
      || /(?:pet$|^sentinel$|^predasite|^vulpaphyla|^helminthcharger|^companion)/.test(normalized);
  }
  ...
  function minimapDisplayFrame(config, verified) {
    const imageWidth = Number.isFinite(Number(config.width)) && Number(config.width) > 0 ? Number(config.width) : 100;
    const imageHeight = Number.isFinite(Number(config.height)) && Number(config.height) > 0 ? Number(config.height) : 100;
    let left = 0, top = 0, right = imageWidth, bottom = imageHeight;
    // Extend only reviewed layouts and only for verified positions. Preserve
    // the image's calibrated scale and leave uncharted entrance geometry blank.
    if (config.fitObservedSpawns) {
      ...
      for (const { position } of verified) {
        ...
        left = Math.min(left, Math.floor(x - 32));
        top = Math.min(top, Math.floor(y - 32));
        right = Math.max(right, Math.ceil(x + 32));
        bottom = Math.max(bottom, Math.ceil(y + 32));
      }
    }
    const width = right - left, height = bottom - top;
    const expanded = left !== 0 || top !== 0 || width !== imageWidth || height !== imageHeight;
    return {
      x: left, y: top, width, height, expanded,
      imageStyle: expanded
        ? `left:${-left/width*100}%;top:${-top/height*100}%;width:${imageWidth/width*100}%;height:${imageHeight/height*100}%;`
        : "",
    };
  }
  ...
  function renderMinimap(run, points) {
    ...
    const frame = minimapDisplayFrame(config, exactTileMatch ? verified : []);
    const mapWidth = frame.width, mapHeight = frame.height;
    const dimensions = `viewBox="${frame.x} ${frame.y} ${mapWidth} ${mapHeight}"`;
    let bubbles = "";
    ...
    const extensionNote = frame.expanded ? `<p class="card-subtitle">Outer spawn positions shown; entrance geometry is not mapped.</p>` : "";
    return `<section class="card"><h3 class="card-title">Tile layout</h3><p class="card-subtitle">${subtitle}</p><div class="minimap-wrap" role="button" tabindex="0" aria-label="Expand tile layout for ${h(config.label)}"><div class="minimap-stage" data-minimap-width="${mapWidth}" data-minimap-height="${mapHeight}" style="aspect-ratio:${mapWidth}/${mapHeight}"><img src="${h(config.src)}" alt="Tile layout for ${h(config.label)}" style="${frame.imageStyle}"><svg class="minimap-overlay" ${dimensions} preserveAspectRatio="none">${bubbles}</svg></div></div>${elevationLegend}${extensionNote}</section>`;
  }

  function syncMinimapStages(root = document) {
    root.querySelectorAll(".minimap-wrap").forEach((wrap) => {
      const stage = wrap.querySelector(":scope > .minimap-stage");
      if (!stage) return;
      const availableWidth = wrap.clientWidth;
      const availableHeight = wrap.clientHeight;
      const mapWidth = Number(stage.dataset.minimapWidth) || 1;
      const mapHeight = Number(stage.dataset.minimapHeight) || 1;
      if (!(availableWidth > 0 && availableHeight > 0 && mapWidth > 0 && mapHeight > 0)) return;
      const scale = Math.min(availableWidth / mapWidth, availableHeight / mapHeight);
      const width = Math.max(1, Math.floor(mapWidth * scale));
      const height = Math.max(1, Math.floor(mapHeight * scale));
      const widthValue = `${width}px`;
      const heightValue = `${height}px`;
      if (stage.style.width !== widthValue) stage.style.width = widthValue;
  ...
  async function prepareRuns(runs, sourceName, sourceDate) {
    await initializeSavedRuns();
    for (const run of runs) {
      run.sourceName = sourceName;
      run.sourceDate = run.sourceDate || sourceDate;
      run.actualVitus = "";
      run.clientFreshBlessing = false;
      try {
        run.runFingerprint = await Parser.fingerprintRun(run);
        run.shortId = run.runFingerprint.slice(0, 12);
        restoreSavedActualVitus(run);
      } catch (_) {
        run.runFingerprint = "";
        run.shortId = "local";
      }
  ...
  function renderRunList() {
    ...
    $("#runList").innerHTML = visible.length ? visible.map(({run,index})=>`<button class="run-button ${!state.activeSavedId && index===state.activeIndex?"active":""}" type="button" data-index="${index}"><span class="run-accent"></span><span class="run-copy"><strong>${h(run.node)}</strong><span>${h(run.missionType)} · ${h(run.planet)}</span></span><span class="run-meta"><strong class="${Number(run.totalDuration) < INITIAL_RUN_MIN_SECONDS ? "short-run-duration" : ""}">${duration(run.totalDuration)}</strong>${fmt(run.droneKills)} drones</span></button>`).join("") : `<div class="no-runs">${state.runs.length ? "No matching runs." : "No analyzed runs."}</div>`;
    $$(".run-button",$("#runList")).forEach((button)=>button.addEventListener("click",()=>{
      state.activeSavedId=""; state.activeIndex=Number(button.dataset.index); renderSavedRunList(); renderRunList(); renderReport(state.runs[state.activeIndex]); focusActualVitusEntry(); document.body.classList.remove("sidebar-open");
    }));
  }

  function renderSavedRunList() {
    const toggle = $("#savedRunsToggle");
    const list = $("#savedRunList");
    if (!toggle || !list) return;
    toggle.setAttribute("aria-expanded", String(state.savedRunsExpanded));
    list.hidden = !state.savedRunsExpanded;
    list.innerHTML = state.savedRuns.length ? state.savedRuns.map((record) => {
      const run = record.run;
      const sourceDate = normalizedRunDate(run);
      const dateTime = sourceDate ? sourceDate.toISOString() : "";
      const dateLabel = localRunDate(run);
      return `<div class="saved-run-row ${state.activeSavedId === record.id ? "active" : ""}" data-saved-id="${h(record.id)}">
        <button class="saved-run-button" type="button" data-saved-open="${h(record.id)}">
          <span class="run-accent"></span>
          <span class="saved-run-copy"><strong>${h(run.node || "Saved run")}</strong><span>${h(run.missionType || "ARBITRATION")} - ${h(run.planet || "Unknown")}</span><time${dateTime ? ` datetime="${h(dateTime)}"` : ""}>${h(dateLabel)}</time></span>
          <span class="run-meta"><strong class="${Number(run.totalDuration) < INITIAL_RUN_MIN_SECONDS ? "short-run-duration" : ""}">${duration(run.totalDuration)}</strong>${fmt(run.droneKills)} drones</span>
        </button>
        <button class="saved-run-delete" type="button" data-saved-delete="${h(record.id)}" aria-label="Remove ${h(run.node || "run")} from saved runs" data-tooltip="Remove saved run"><svg width="15" height="15" aria-hidden="true"><use href="#icon-trash"></use></svg></button>
      </div>`;
    }).join("") : `<div class="no-runs saved-runs-empty">No saved runs yet.</div>`;
    $$("[data-saved-open]", list).forEach((button) => button.addEventListener("click", () => {
      const record = state.savedRuns.find((saved) => saved.id === button.dataset.savedOpen);
      if (!record) return;
      state.activeSavedId = record.id;
      renderSavedRunList();
      renderRunList();
      // Reopening a saved run is an explicit refresh, even if this object was
      // already viewed. Start before rendering to avoid a second auto-load.
      void ensureTilesetAverages(record.run);
      renderReport(record.run);
      focusActualVitusEntry();
      document.body.classList.remove("sidebar-open");
    }));
    $$("[data-saved-delete]", list).forEach((button) => button.addEventListener("click", () => {
      void removeSavedRun(button.dataset.savedDelete);
    }));
    setupAnalyzerTooltips(list);
  }

  function formatParseSummary(bytes, elapsedMs, runCount) {
    const size = bytes >= 1e9 ? `${(bytes / 1e9).toFixed(1)}gb` : `${(bytes / 1e6).toFixed(1)}mb`;
    const seconds = (Math.max(0, elapsedMs) / 1000).toFixed(elapsedMs < 1000 ? 3 : 1);
    return `${size} Parsed in ${seconds}s - ${runCount} run${runCount === 1 ? "" : "s"} found.`;
  }
  ...
  async function importFile(file, settleFocus = false) {
    ...
    try {
      ...
      status.className="parse-status success"; status.textContent=formatParseSummary(file.size, parseElapsedMs, runs.length);
      showToast(`Loaded ${runs.length} local run${runs.length===1?"":"s"}.`);
      void reportSpawnMetrics(runs, {
        force: runs.some((run) => run.resourceBlessingRefreshUnconfirmed
          || (!Number.isFinite(run.blessedDroneKills)
            && Number(run.totalDuration) > RESOURCE_BLESSING_SECONDS)),
      });
  ...
  function reportSpawnMetrics(runs, options = {}) {
    const buildContribution = options.buildContribution
      || ((target) => Parser.buildContribution(target, { blessedDroneKills: effectiveBlessedDroneKills(target) }));
    const queued = metricsSubmissionQueue.then(() => SpawnSubmission.submitRuns(runs, buildContribution, options));
    metricsSubmissionQueue = queued.catch(() => null);
    return queued.catch(() => null);
  }

  function scheduleBlessingMetricSync(run) {
    let sync = blessingMetricSync.get(run);
    if (!sync) {
      sync = { timer: 0, inFlight: false, pending: false, lastBlessedDroneKills: null };
      blessingMetricSync.set(run, sync);
    }
    if (sync.timer) clearTimeout(sync.timer);
    sync.timer = setTimeout(() => flushBlessingMetricSync(run), BLESSING_METRIC_SYNC_DELAY_MS);
  }

  async function flushBlessingMetricSync(run) {
    const sync = blessingMetricSync.get(run);
    if (!sync) return;
    sync.timer = 0;
    const blessedDroneKills = Math.max(0, Math.min(
      Math.trunc(run.droneKills || 0),
      Math.trunc(effectiveBlessedDroneKills(run) ?? run.droneKills ?? 0),
    ));
    if (sync.inFlight) {
      sync.pending = true;
      return;
    }
    if (sync.lastBlessedDroneKills === blessedDroneKills) return;
    sync.inFlight = true;
    sync.pending = false;
    const result = await reportSpawnMetrics([run], {
      force: true,
      buildContribution: (target) => Parser.buildContribution(target, { blessedDroneKills }),
    });
    sync.inFlight = false;
    if (result && result.failed === 0 && !result.disabled) sync.lastBlessedDroneKills = blessedDroneKills;
    const currentBlessedDroneKills = Math.max(0, Math.min(
      Math.trunc(run.droneKills || 0),
      Math.trunc(effectiveBlessedDroneKills(run) ?? run.droneKills ?? 0),
    ));
    if (sync.pending || currentBlessedDroneKills !== blessedDroneKills) {
      if (sync.timer) clearTimeout(sync.timer);
      sync.timer = setTimeout(() => flushBlessingMetricSync(run), BLESSING_METRIC_SYNC_DELAY_MS);
    }
  }
  ...
  function buildCorrelationMetrics(run) {
    const rotationPhases = Parser.helpers.calculateRotationPhases(run);
    const fixedIntervals = run.missionType === "DISRUPTION"
      || (run.missionType === "VOID CASCADE" && !rotationPhases.length);
    ...
    const phases = fixedIntervals
      ...
      : rotationPhases;
    const blessingExpiry = effectiveBlessingExpiry(run);
    const canUseClientFreshBlessingOverride = canUseClientFreshBlessing(run);
    if (!phases.length) return { phases: [], metrics: [], phaseNoun, axisPrefix, fixedIntervals, blessingExpiry, canUseClientFreshBlessing: canUseClientFreshBlessingOverride, clientFreshBlessing: run.clientFreshBlessing === true, resourceBlessingRefreshUnconfirmed: run.resourceBlessingRefreshUnconfirmed === true };
    ...
    const threshold = ["SURVIVAL", "DISRUPTION", "VOID CASCADE"].includes(run.missionType) ? 30 : 15;
    ...
    const definitions = [
      ...
      { id: "enemiesPerDrone", name: "Enemies / drone", color: "#f472b6", dash: "8 3 2 3", values: enemyValues.map((count, index) => droneValues[index] ? Math.max(0, count - droneValues[index]) / droneValues[index] : null), digits: 2, avgDigits: 2 },
    ...
    return { phases, metrics, phaseNoun, axisPrefix, fixedIntervals, blessingExpiry, canUseClientFreshBlessing: canUseClientFreshBlessingOverride, clientFreshBlessing: run.clientFreshBlessing === true, resourceBlessingRefreshUnconfirmed: run.resourceBlessingRefreshUnconfirmed === true };
  }
  ...
  function renderCorrelationCard(data) {
    ...
    const blessingLine = Number.isFinite(blessingX)
      ...
      : "";
    const refreshUnconfirmed = data.resourceBlessingRefreshUnconfirmed && !data.clientFreshBlessing;
    const expiryText = data.blessingExpiry
      ? (data.blessingExpiry.expiredBeforeRun ? "Blessing had expired before this run"
        : `${refreshUnconfirmed && data.blessingExpiry.assumedFromMissionStart ? "Assumed Blessing expiry at:" : "Blessing ran out at:"} ${blessingDuration(data.blessingExpiry.elapsed)}`)
      : "";
    const blessingText = refreshUnconfirmed
      ? `Refresh unconfirmed${expiryText ? ` · ${expiryText}` : " before mission"}`
      : expiryText;
    const blessingLabel = blessingText
      ? `<span class="correlation-blessing-expiry-label">${h(blessingText)}</span>`
      : "";
    const clientBlessingButton = data.canUseClientFreshBlessing
      ? `<button class="client-fresh-blessing-button${data.clientFreshBlessing ? " is-active" : ""}" type="button" data-client-fresh-blessing aria-pressed="${data.clientFreshBlessing}" data-html2canvas-ignore="true">Click this button if Client had Fresher Blessing</button>`
      : "";
    const blessingStatus = blessingLabel || clientBlessingButton
      ? `<div class="correlation-blessing-status">${blessingLabel}${clientBlessingButton}</div>`
      : "";
  ...
  function setupCorrelationInteractions(root) {
    ...
    const run = card?._correlationRun;
    if (!card || !data) return;
    const clientBlessingButton = card.querySelector("[data-client-fresh-blessing]");
    if (clientBlessingButton && run) {
      clientBlessingButton.addEventListener("click", () => {
        run.clientFreshBlessing = !run.clientFreshBlessing;
        renderReport(run);
        scheduleBlessingMetricSync(run);
        showToast(run.clientFreshBlessing
          ? "Using the client's fresh three-hour Blessing for this run."
          : "Using the Blessing timing recorded for the host.");
      });
    }
  ...
  function prepareCorrelationLayout(report, run) {
    ...
    correlation._correlationRun = run;
  ...
  async function copyReportImage() {
    ...
    try {
      ...
      await new Promise((resolve) => requestAnimationFrame(resolve));
      syncReportHeaderAccent(report);
      const captureBounds = measureExportStage(stage, report);
      stage.style.width = `${captureBounds.width}px`;
      stage.style.height = `${captureBounds.height}px`;
      await new Promise((resolve) => requestAnimationFrame(resolve));
      ...
      const canvas = await html2canvas(stage, {
        ...
        windowWidth: 2022,
        windowHeight: Math.max(captureBounds.height, 1080),
      });
  ...
  function openMinimapLightbox(trigger) {
    ...
    syncMinimapStages(body);
    requestAnimationFrame(() => syncMinimapStages(body));
  }
  ...
  function bindEvents() {
    ...
    let pageDragDepth=0;
    let pageDragReleaseTimer=null;
    const clearEmptyPageDrag=()=>{
      pageDragDepth=0;
      if(pageDragReleaseTimer!==null){clearTimeout(pageDragReleaseTimer);pageDragReleaseTimer=null;}
      document.body.classList.remove("empty-page-drag");
    };
    const armEmptyPageDragRelease=()=>{
      if(pageDragReleaseTimer!==null) clearTimeout(pageDragReleaseTimer);
      pageDragReleaseTimer=setTimeout(clearEmptyPageDrag,750);
    };
    input.addEventListener("change",()=>importFile(input.files[0]));
    $("#clearRunsBtn").addEventListener("click", clearRuns);
    $("#savedRunsToggle").addEventListener("click", () => {
      state.savedRunsExpanded = !state.savedRunsExpanded;
      renderSavedRunList();
    });
    $("#tilesetAverageToggle").addEventListener("click", () => {
      state.showTilesetAverages = !state.showTilesetAverages;
      saveTilesetAverageVisibility();
      syncTilesetAverageToggle();
      const run = currentDisplayedRun();
    ...
    document.addEventListener("dragenter",(event)=>{
      ...
      armEmptyPageDragRelease();
    });
    document.addEventListener("dragover",(event)=>{
      if(!isFileDrag(event)) return;
      event.preventDefault();
      event.dataTransfer.dropEffect="copy";
      if(!state.runs.length&&document.body.classList.contains("empty-page-drag")) armEmptyPageDragRelease();
    });
    document.addEventListener("dragleave",(event)=>{
      if(!document.body.classList.contains("empty-page-drag")) return;
      const leftViewport=event.target===document||event.target===document.documentElement||event.clientX<=0||event.clientY<=0||event.clientX>=innerWidth||event.clientY>=innerHeight;
      if(leftViewport){clearEmptyPageDrag();return;}
      pageDragDepth=Math.max(0,pageDragDepth-1);
      if(!pageDragDepth) clearEmptyPageDrag();
      else armEmptyPageDragRelease();
    });
    ...
    document.addEventListener("dragend",clearEmptyPageDrag);
    addEventListener("blur",clearEmptyPageDrag);
    document.addEventListener("pointerdown",(event)=>{
      const selector=document.querySelector(".vitus-style-selector[open]");
      if(selector&&!selector.contains(event.target)) selector.removeAttribute("open");
    });
    ...
    $("#resetControlsBtn").addEventListener("click",()=>{state.fontStep=0;state.spacingStep=0;state.widthStep=0;saveDisplayPreferences();applyDisplayControls();});
