(function () {
  ...
  function renderVitus(run) {
    ...
    const subtitle = run.resourcefulRetrieverDetected
      ? `<p class="card-subtitle">100% pickup, both boosters, Resourceful Retriever on.</p>`
      : `<p class="card-subtitle vitus-retriever-warning">!! MISSING RESOURCEFUL RETRIEVER MOD !!</p>`;
    return `<section class="card vitus-card"><h3 class="card-title">Expected Vitus</h3>${subtitle}<div class="highlight-panel vitus-entry-panel"><div class="vitus-entry-group"><span class="vitus-entry-label">Actual Vitus</span><input id="actualVitusInput" class="vitus-input" type="text" inputmode="numeric" pattern="[0-9]*" autocomplete="off" placeholder="enter" value="${h(run.actualVitus || "")}"><span id="vitusRate" class="vitus-rate">${h(formatVitusRate(run))}</span></div><div id="vitusLuck" class="vitus-luck" style="--luck-color:${luckColor}"><strong>${h(classified.label)}</strong><div class="mini">expected ${fmt(result.mean)}</div></div></div><table class="vitus-table"><thead><tr><th>CHANCE</th><th>TOTAL</th><th>LUCK LEVEL</th></tr></thead><tbody>${result.scenarios.map((scenario) => `<tr class="${scenario === classified ? "active" : ""}"><td>${scenario.chance}</td><td><strong>${fmt(scenario.total)}</strong></td><td>${h(scenario.label)}</td></tr>`).join("")}</tbody></table></section>`;
  }
