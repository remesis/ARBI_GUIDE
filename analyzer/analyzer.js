(function () {
  ...
  function phaseInfo(run) {
    ...
    const items = defense ? run.waveDurations.map(([label, seconds], index) => ({ label: String(label), seconds, saturation: saturation[index] }))
      : (run.rotationDurations || []).map((seconds, index) => ({ label: `R${index + 1}`, seconds, saturation: saturation[index] }));
  ...
  function renderClearMap(run, phase) {
    ...
    const cells = phase.items.map((item) => {
      ...
      return `<div class="heat-cell" title="Saturation: ${h(saturation)}" aria-label="${h(item.label)}, saturation ${h(saturation)}" style="--heat:${color.color};--ink:${color.ink}">${content}</div>`;
    }).join("");
  ...
  function renderDpm(run) {
    ...
    const width = 520, height = 190, pad = { l: 34, r: 15, t: 15, b: 25 };
  ...
  function renderVitus(run) {
    ...
    return `<section class="card vitus-card"><h3 class="card-title">Expected Vitus</h3><p class="card-subtitle">100% pickup, all buffs, Resourceful Retriever on.</p><div class="highlight-panel"><div class="kpi-label">Actual Vitus</div><div style="display:flex;align-items:end;justify-content:space-between;gap:12px"><input id="actualVitusInput" class="vitus-input" type="number" min="0" placeholder="enter" value="${h(run.actualVitus || "")}"><div id="vitusLuck" style="text-align:right"><strong>${h(nearest.label)}</strong><div class="mini">expected ${fmt(result.mean)}</div></div></div></div><table class="vitus-table"><thead><tr><th>CHANCE</th><th>TOTAL</th><th>LUCK LEVEL</th></tr></thead><tbody>${result.scenarios.map((scenario) => `<tr class="${scenario === nearest ? "active" : ""}"><td>${scenario.chance}</td><td><strong>${fmt(scenario.total)}</strong></td><td>${h(scenario.label)}</td></tr>`).join("")}</tbody></table></section>`;
  }
