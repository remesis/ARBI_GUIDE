...
import {unpackCatalog} from './catalog.mjs';
...
const nameOf = id => definitions.find(t => t.id === id)?.name || '';
...
const allowed = (id, polarity) => family.traits[id] && family.traits[id][polarity] !== 'excluded';
...
function render({selectBest = false} = {}) {
  ...
  $('#poolSummary').textContent = `${intervalText(research.p)} positive traits · ${intervalText(research.n)} negative traits`;
...
function renderResults() {
  ...
  rec.classList.toggle('is-uncertain', incomplete || research.winningLock === 'uncertain');
  const bestText = incomplete ? 'Choose an acceptable negative' : !state.hasNegative ? 'Lock a positive' : research.winningLock === 'tie' ? 'Both locks have equal odds' : research.winningLock === 'uncertain' ? 'The best lock depends on eligibility' : `Lock a ${research.winningLock}`;
  const description = incomplete ? 'Select at least one alternative, or choose No negative.' : !state.hasNegative ? 'A locked negative cannot produce a 0N target.' : research.winningLock === 'negative' ? `Keep ${nameOf(state.heldNegative)} while rolling the ${target().positives.length} positives.` : research.winningLock === 'positive' ? 'Keep any selected positive and accept the chosen negative alternatives.' : 'Compare the probability ranges below. Both locked strategies use the same Kuva cost.';
  rec.innerHTML = `<span class="recommendation-label">${incomplete ? 'COMPLETE YOUR TARGET' : 'BEST LOCK FOR THIS TARGET'}</span><strong>${esc(bestText)}</strong><p>${esc(description)}</p>`;
...
function renderCrossovers() {
  const section = $('.crossover-section'); section.hidden = !state.hasNegative || !state.negatives.length;
