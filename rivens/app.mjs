...
import {unpackCatalog, COMBINED_TRAITS, isCombinedTrait, splicedTraitsFor, splicedTrait, SPLICE_BASELINE_SOURCE, spliceRecipes} from './catalog.mjs?v=20260924-splice-range-fallback';
...
function renderRanges() {
  ...
  if (splices.length) {
    rows.push(`<section class="range-section" aria-label="Spliced stat ranges"><h3>Spliced stats<span>Positive only</span></h3><p class="range-empty splice-range-note">Provisional <a href="${SPLICE_BASELINE_SOURCE.url}" target="_blank" rel="noopener noreferrer">wiki baselines</a> · ${SPLICE_BASELINE_SOURCE.date}. Missing elemental/faction values use ordinary-stat ranges; other missing values remain unknown.</p>`);
...
function renderSelectedLockSetup() {
  ...
  section.innerHTML = heading + `
    ...
    <p class="cost-caption">Uses uniform grades across the ±10% band, independent of trait selection, under the positives-first model. ${pools.length > 1 ? 'Odds are bounds across unresolved eligible pools. ' : ''}${splice ? 'Spliced traits retain their grade. Wiki baselines take priority; only missing elemental/faction values currently use ordinary-stat ranges. Other missing values remain unknown. ' : ''}This step does not require the rest of the final target.</p>`)}
    </div></details>`;
}
