...
import {isCombinedTrait, splicedTraitsFor} from './catalog.mjs?v=20260924-shared-splice-ranges';
...
export function renderMath({catalog, research, target, variant, format, nameOf}) {
  ...
  <p><strong>Provisional splice ranges.</strong> Elemental and faction splices currently assume the same ranges as their corresponding ordinary stats for the selected weapon and variant, including its disposition, format, grade band and rounding. Other numeric baselines come from the <a href="https://wiki.warframe.com/w/Riven_Mods#Spliced_Values" target="_blank" rel="noopener noreferrer">wiki's Spliced Values table</a>, reviewed 24 September 2026; the source flags some values and names as placeholders, and their display rounding is provisional. Remaining missing weapon-type values and Parry Angle's unclear unit stay unknown. These entries are not added to ordinary cycling pools.</p>
