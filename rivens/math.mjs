...
import {isCombinedTrait, splicedTraitsFor} from './catalog.mjs?v=20260922-splicing';
...
export function renderMath({catalog, research, target, variant, format, nameOf}) {
  ...
  <p><strong>Splicing</strong> creates a positive trait from an announced recipe. The two ingredients are replaced by the splice and a random ordinary trait, preserving the original positive/negative layout. A retained splice then preserves that format on every cycle, even with no manual lock. Selecting a splice here models an already-spliced Riven; it does not estimate creating one. Splicing launches later with Glacial Defiance.</p>
